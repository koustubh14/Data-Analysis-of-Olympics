...
def data_over_time(df, col):
    ...
    #nations_over_time.columns = ['Edition', col]  # Explicitly set column names
    #nations_over_time = nations_over_time.sort_values('Edition')
    #return nations_over_time
    # New implementation using groupby and nunique
    over_time = df.groupby('Year')[col].nunique().reset_index()
    over_time.columns = ['Edition', col]  # Proper column naming
    return over_time.sort_values('Edition')
...
def most_successful(df, sport):
    ...
    x = medal_counts.head(15).merge(df, on='Name', how='left')[
...
def most_successful_countrywise(df, country):
    # Filter rows with medals and for the specified country
    temp_df = df.dropna(subset=['Medal'])
    ...
    merged = top_10.merge(df, on='Name', how='left')
